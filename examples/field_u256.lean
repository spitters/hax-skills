...
/-- A 256-bit unsigned integer represented as 4 × 64-bit limbs (little-endian), a length-indexed `Vector` -/
structure U256 where
  limbs : Vector UInt64 4
  deriving Repr, DecidableEq

namespace U256

/-- Zero constant -/
def zero : U256 := ⟨#v[0, 0, 0, 0]⟩

/-- One constant -/
def one : U256 := ⟨#v[1, 0, 0, 0]⟩

/-- Create from a single u64 -/
def fromU64 (value : UInt64) : U256 := ⟨#v[value, 0, 0, 0]⟩
...
def ctSelect (a b : U256) (choice : UInt64) : U256 :=
  ...
  ⟨#v[a.limbs[0] ^^^ (mask &&& (a.limbs[0] ^^^ b.limbs[0])),
